// Version 15 enhancements: page sub-navigation and Tanzania shopping checklist.
(function () {
  'use strict';

  const pageMenus = {
    'index.html': [
      ['Dashboard', '#dashboard'], ['Weather', '#weather'], ['Trip checklist', '#tripcheck']
    ],
    'trip.html': [
      ['Trip overview', '#trip'], ['Flights', '#flights'], ['Itinerary', '#itinerary'], ['Tipping', '#tipping']
    ],
    'shoot.html': [
      ['Shoot Now', '#shootnow'], ['Photo banks', '#photo'], ['Video banks', '#video'], ['Field setup', '#setup']
    ],
    'wildlife.html': [
      ['Wildlife', '#wildlife'], ['Species', '#species'], ['Zanzibar', '#zanzibar'], ['Activities', '#activities']
    ],
    'more.html': [
      ['Shopping', '#shopping'], ['Protection', '#travelprotection'], ['Documents', '#documents'], ['To-dos', '#todos'], ['Contacts', '#contacts'], ['Library', '#library']
    ]
  };
  ...
  function firstExistingTarget(items) {
    return items.filter(([, href]) => document.querySelector(href));
  }

  function installPageMenu() {
    if (document.querySelector('.page-subnav')) return;
    const items = firstExistingTarget(pageMenus[currentPage()] || []);
    ...
    if (topnav) topnav.insertAdjacentElement('afterend', nav);
  ...
  <section class="section searchable shopping-section" id="shopping">
    ...
    <div class="shopping-rule"><b>Final buying rule:</b> If the color doesn’t immediately impress me, I won’t buy it. A single exceptional gemstone is far better than several average ones.</div>

    <details open><summary>🥇 Priority 1 — AAA Tanzanite</summary><div class="body">
      ...
      <div class="shop-checks">
        ...
        <label><input type="checkbox" data-shopcheck="tanzanite-clean"> Eye-clean, no visible inclusions</label>
    ...
    <details><summary>⭐ Priorities 5 &amp; 6 — Tanzanian Sapphire and Blue Spinel</summary><div class="body">
      <h3>Tanzanian Sapphire</h3><p><b>Budget:</b> $1,000–3,000 · <b>Size:</b> 1.5–3 ct · <b>Price:</b> $800–3,000/ct</p>
      <div class="shop-checks"><label><input type="checkbox" data-shopcheck="sapphire-color"> Cornflower or rich royal blue</label><label><input type="checkbox" data-shopcheck="sapphire-clean"> Eye-clean</label><label><input type="checkbox" data-shopcheck="sapphire-cut"> Excellent cut</label><label><input type="checkbox" data-shopcheck="sapphire-certified"> Certified</label></div>
      <h3>Blue Spinel</h3><p><b>Budget:</b> $800–2,500 · <b>Size:</b> 2–3 ct · <b>Price:</b> $700–3,000/ct</p>
      <div class="shop-checks"><label><input type="checkbox" data-shopcheck="blue-pure"> Pure blue</label><label><input type="checkbox" data-shopcheck="blue-sparkle"> Bright sparkle</label><label><input type="checkbox" data-shopcheck="blue-even"> Even color</label><label><input type="checkbox" data-shopcheck="blue-clean"> Eye-clean</label><label><input type="checkbox" data-shopcheck="blue-certified"> Certified</label></div>
    </div></details>

    <details><summary>🛍️ Authentic Tanzanian Souvenirs</summary><div class="body">
      <h3>Arusha</h3><div class="shop-checks"><label><input type="checkbox" data-shopcheck="makonde"> Makonde ebony wood carving ($100–500)</label><label><input type="checkbox" data-shopcheck="tingatinga"> Tinga Tinga painting ($50–300)</label><label><input type="checkbox" data-shopcheck="maasai"> Maasai handmade bead jewelry ($20–100)</label></div>
      <h3>Zanzibar / Stone Town</h3><div class="shop-checks"><label><input type="checkbox" data-shopcheck="spices"> Premium spice gift set ($20–60): cinnamon, cardamom, vanilla, cloves, black pepper and nutmeg</label><label><input type="checkbox" data-shopcheck="wood-box"> Hand-carved wooden box ($30–150)</label><label><input type="checkbox" data-shopcheck="fabric"> Kikoy or Kanga fabric ($15–60)</label><label><input type="checkbox" data-shopcheck="leather"> Handmade leather goods—inspect quality carefully</label></div>
    </div></details>

    <details><summary>❌ Do Not Buy</summary><div class="body"><div class="shop-checks danger-list">
      <label><input type="checkbox" data-shopcheck="avoid-ivory"> Ivory</label><label><input type="checkbox" data-shopcheck="avoid-turtle"> Sea turtle shell</label><label><input type="checkbox" data-shopcheck="avoid-coral"> Coral</label><label><input type="checkbox" data-shopcheck="avoid-uncertified"> Uncertified gemstones</label><label><input type="checkbox" data-shopcheck="avoid-street"> Stones from street vendors</label><label><input type="checkbox" data-shopcheck="avoid-pressure"> “Today only” deals</label>
    </div></div></details>
    ...
    <details><summary>Negotiation Script</summary><div class="body">
      <p>“We’re comparing a few reputable jewelers before making our decision.”</p><p>“If we purchase today, what is your best price?”</p><p>“Is there a discount for cash or if we buy multiple items?”</p><p><b>Expected negotiation range:</b> approximately 5–15% on quality certified gemstones.</p>
    </div></details>
    ...
    <details open><summary>Planned Budget — $15,000</summary><div class="body"><div class="budget-table">
      <div><b>AAA Tanzanite</b><span>$5,000</span></div><div><b>Vivid Red Spinel</b><span>$4,000</span></div><div><b>Mahenge Pink Spinel</b><span>$2,500</span></div><div><b>Tsavorite Garnet</b><span>$2,000</span></div><div><b>Tanzanian Sapphire or Blue Spinel</b><span>$1,500</span></div>
    </div></div></details>
  </section>`;
  ...
  function installShopping() {
    ...
    const key = 'z8-tanzania-shopping-v15';
    let saved = {};
    try { saved = JSON.parse(localStorage.getItem(key) || '{}'); } catch (_) { saved = {}; }
    document.querySelectorAll('[data-shopcheck]').forEach(cb => {
      cb.checked = !!saved[cb.dataset.shopcheck];
      cb.addEventListener('change', () => {
        saved[cb.dataset.shopcheck] = cb.checked;
        try { localStorage.setItem(key, JSON.stringify(saved)); } catch (error) { console.error(error); }
      });
    });
  }
